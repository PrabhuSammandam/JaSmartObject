...
namespace ja_iot
{
  namespace base
  {
    ...
    public:
      ...
      PtrArray(T* item_base_ptr, const uint16_t capacity) : _list{item_base_ptr}, _capacity{capacity}
      {
        if (item_base_ptr != nullptr)
        {
      ...
      void Remove(T list_entry)
      {
        for (uint16_t i = 0; i < _capacity; i++)
        {
          if (_list[i] == list_entry)
          {
            _count--;
            _list[i] = nullptr;
            break;
      ...
      T GetItem(const uint16_t index)
      {
