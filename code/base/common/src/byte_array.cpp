...
ByteArray::ByteArray( uint16_t capacity ) : _capacity{ capacity }
{
  if( _capacity > 0 )
  {
    _byte_array_ptr = static_cast<uint8_t *>( mnew_g( capacity ) );
  }
}

ByteArray::ByteArray( uint8_t *buf, uint16_t buf_len, bool is_mem_owned )
{
...
uint8_t ByteArray::operator [] ( uint16_t index ) const
{
...
ByteArray * ByteArray::set_len( uint16_t len )
{
  _length = len;
  return ( this );
}
...
void ByteArray::copy_n_bytes( uint8_t *src_buf, uint16_t len )
{
...
void ByteArray::copy_to( ByteArray *dst, uint16_t from, uint16_t len ) const
{
  ...
  auto     dstBuf = dst->get_array();
  uint16_t i      = 0;

  for(; ( i < len ) && ( ( i + from ) < _length ) && ( i < dst->_capacity ); i++ )
  {
    dstBuf[i] = _byte_array_ptr[i + from];
  }
...
void ByteArray::set_array( uint8_t *buf, uint16_t bufLen, bool freeBuf )
{
...
uint32_t ByteArray::get_hash_value()
{
