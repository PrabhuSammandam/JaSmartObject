  ...
  namespace network {
    class Endpoint {
      public:
        ...
        Endpoint( uint16_t u16_adapter_type, uint16_t u16_network_flags, uint16_t port,
          uint32_t u32_if_index, const base::IpAddress &rcz_ip_addr );

        uint16_t get_adapter_type() const
        {
          return ( _u16_adapter_type );
        }
        void set_adapter_type( _in_ const uint16_t adapter_type )
        {
          _u16_adapter_type = adapter_type;
        }
        uint32_t get_if_index() const
        {
          return ( _u32_if_index );
        }
        void set_if_index( _in_ const uint32_t if_index )
        {
          _u32_if_index = if_index;
        }
        uint16_t get_network_flags() const
        {
          return ( _u16_network_flags );
        }
        void set_network_flags( _in_ const uint16_t network_flags )
        {
          _u16_network_flags = network_flags;
        }
        uint16_t get_port() const
        {
          return ( _u16_port );
        }
        void set_port( _in_ const uint16_t port )
        {
          _u16_port = port;
        }
        base::IpAddress& get_addr()
        {
          return ( _cz_ip_addr );
        }
      ...
      private:
        uint16_t        _u16_adapter_type  = base::k_adapter_type_default;
        uint16_t        _u16_network_flags = base::k_network_flag_none;
