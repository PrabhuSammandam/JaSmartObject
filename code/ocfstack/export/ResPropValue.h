...
class ResRepresentation
{
  public:
    ...
    std::map<std::string, ResPropValue *>& get_props()
    {
      return ( _props );
    }
  ...
  private:
    std::map<std::string, ResPropValue *>   _props;
};
...
template<> struct res_prop_type_trait<std::vector<int> >
{
  static constexpr ResPropValType   PROP_VAL_TYPE = ResPropValType::integer_array;
  typedef std::vector<int> PROP_VALUE;
};
...
template<>
struct PropTraits<int> : res_prop_type_trait<int>
{
...
template<>
struct PropTraits<std::vector<int> > : res_prop_type_trait<std::vector<int> >
{
